...
- [x] 1.2 将 `site/resources/README.md` 改为资源目录默认页，公开页面只保留更新日期和资源组件。
...
- [x] 2.3 更新 `site/.vuepress/collections/resources.ts`，中文资源中心侧边栏只保留默认页。
- [x] 2.4 确认资源中心默认页 frontmatter 包含 `title`、`permalink`、`date`，并关闭默认阅读时间、页面日期、侧边栏和右侧目录；路由为 `/resources/`。
- [x] 2.5 如使用外部 logo/icon，优先使用稳定公开 URL；无法可靠确认时使用文本、标签或 GitHub owner avatar，避免下载或提交未授权商业截图。

## 3. 验证

- [x] 3.1 检查 `/resources/` 页面路径可访问并默认展示资源目录。
- [x] 3.2 检查资源中心侧边栏不再展示其他中文资源文章入口。
