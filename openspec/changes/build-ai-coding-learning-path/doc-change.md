...
- 涉及栏目：`learn`
- 涉及页面：`/learn/`、`/learn/ai-coding-intro/`、`/learn/tools-and-models/`、`/learn/dev-environment/`、`/learn/codex-setup/`、`/learn/first-ai-coding-project/`
- 涉及导航/侧边栏：是
...
- 核心结论：学习路径先帮助读者跑通 AI Coding 入门闭环，再引导进入 OpenSpec、Compound、aisee 等进阶方法论。
- 建议结构：
  1. 这条路径适合谁：AI Coding 新手、用过 AI 工具但效果不稳定的人、准备把 AI Coding 引入团队流程的人。
  2. 两段式路径：入门准备、第一次实战项目。
  3. 五篇正文入口：AI Coding 介绍、工具与模型、运行环境、Codex 安装配置、第一个项目。
  4. 学完能做什么：能让 AI 读项目、改代码、跑命令、看 diff、完成一个简单网页项目。
  5. 进阶入口：Spec-first、OpenSpec、Compound、aisee、工程流程。
- 排版建议：使用路线图、入口卡片、步骤列表和提示块；避免写成纯目录页。
- 视觉要求：必须提供学习路线图或入口卡片组，清楚展示“入门准备 -> 第一个项目 -> 进阶方法论”的关系。
...
- 建议结构：
  ...
  3. 重点比较对象：Codex、Claude Code、Gemini、GLM/Z Code。
  4. 选择矩阵：初学者、真实项目、UI 项目、团队使用分别看什么。
  5. 为什么本站重点讲 Codex：桌面体验、仓库/终端/diff/线程整合，以及后续 worktree、skills、自动化和 OpenSpec 工作流承接。
- 必须使用 logo：通过 LobeHub Icons 提供 OpenAI/Codex、Anthropic/Claude、Google/Gemini、Zhipu/GLM 的 provider 或 model logo。
- 排版建议：使用 logo 卡片、选择矩阵、能力维度表、更新时间提示块。
- 视觉要求：必须包含带 logo 的工具/模型卡片组，并至少包含一个选择矩阵或能力雷达/对比表。
...
- 侧边栏：`learnCollection.sidebar` 需要加入 `ai-coding-intro`、`tools-and-models`、`dev-environment`、`codex-setup`、`first-ai-coding-project`。
- 路由：新增 5 个稳定 permalink，建议分别为 `/learn/ai-coding-intro/`、`/learn/tools-and-models/`、`/learn/dev-environment/`、`/learn/codex-setup/`、`/learn/first-ai-coding-project/`。
...
- 搜索影响：新增正文会进入站内搜索；标题和小节应使用可检索中文关键词，如 AI Coding、Codex、Claude、Gemini、GLM、运行环境、复刻网站。
