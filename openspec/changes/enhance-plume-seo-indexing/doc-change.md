...
| `site/.vuepress/client.ts` | 已注册站点自定义组件。 | 尚未通过 Plume Layout slot 引入 `PageContextMenu`，页面缺少复制页面、查看 Markdown 和打开 LLM 工具的上下文菜单。 |
...
| 修改 | Plume 客户端布局 | `site/.vuepress/client.ts` | 引入 `Layout` 和 `PageContextMenu`，通过 `doc-title-after` slot 在文档页标题旁展示 LLM 友好页面菜单。 |
...
- 构建/部署影响：生产构建会额外生成 sitemap、LLM 友好 Markdown 文件和增强 SEO meta；`hostname` 使用 `https://aisee.wiki`；页面上下文菜单依赖 `llmstxt` 产物。
...
- [ ] 抽查构建产物中存在 `PageContextMenu` 相关脚本或页面上下文菜单文本。
