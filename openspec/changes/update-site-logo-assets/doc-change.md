...
| `LICENSE` | 仓库元数据声明 MIT，但仓库根目录缺少实际 LICENSE 文件。 | GitHub 首页与开源协作入口无法直接展示有效协议文本。 |
...
| 删除 | 重复中文 README | `README.zh-CN.md` | 删除与根 README 重复的中文文件，避免双份维护和语言策略混乱。 |
| 新增 | 开源协议 | `LICENSE` | 新增 MIT License 正文，并在 README 首屏展示协议 badge 与协议说明。 |
...
|---|---|---|---|---|
| 全站配置 | 站点维护者 | 品牌资产目录、品牌色 token、README 首屏简介与全局引用规则 | 本地 `brand/` 目录；favicon 使用无字版本；导航栏使用带字版；组件强调色基于 logo 主品牌色；README 简介体现方法论定位；默认入口采用中文优先；LICENSE 明确为 MIT | 品牌规范手册、重设计方案、PWA 图标矩阵 |
