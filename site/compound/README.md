...
后续正文会按工程循环分组，而不是照搬插件命令清单。

| 分组 | 关注问题 | 后续文章 |
|---|---|---|
| 总览 | Compound Engineering 是什么，为什么它不是单个 `/ce-compound` 命令。 | Compound Engineering 是什么、工程复利核心循环 |
| 上游 | 模糊想法如何变成可执行计划。 | 策略、构思与计划 |
| 交付 | 计划如何进入执行、调试、评审和优化。 | 执行、调试与评审 |
| 多智能体 | 多个 reviewer、document reviewer、design reviewer 和 research agents 如何分工。 | 多智能体评审、Reviewer Personas、文档评审智能体、设计与 UI 评审、Research Agents、多智能体如何协作、评审边界 |
| 知识 | 一次解决如何沉淀为团队知识资产。 | 知识如何复利 |
| 关系 | Compound Engineering 如何与 OpenSpec、aisee、工程流程配合。 | 与 OpenSpec、aisee 如何配合 |

这些文章会逐步补齐。当前页先建立栏目心智模型：把 Compound Engineering 视为一套围绕 AI 工程任务的复利循环，而不是一组孤立命令。
...
插件能力会随版本演进。本站只引用相对稳定的方法论和职责边界；涉及安装方式、命令名称、skill/agent 数量或具体行为时，请以官方 README、公开文档和插件源码为准。

## 下一步阅读

- [OpenSpec](/openspec/)
- [工程流程](/workflows/)
- [术语表](/resources/glossary/)
